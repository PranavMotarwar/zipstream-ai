...
Recent research has explored leveraging large language models (LLMs) for structured data interaction, including natural language interfaces for visualization generation, analytical reasoning, and API-driven code synthesis [@narechania2020nl4dv; @li2024llm; @wang2023large]. These approaches typically assume that data has already been extracted, cleaned, and loaded into tabular formats, leaving the upstream challenges of accessing and parsing compressed datasets unaddressed. Other systems have investigated LLM-assisted data preparation and retrieval-augmented generation (RAG) pipelines, but they similarly rely on preprocessed inputs [@lewis2020retrieval; @gao2023retrieval].
